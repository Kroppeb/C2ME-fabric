...
public final class ChunkDataDeserializer {
    ...
    private static <T> Tick<T> readTick(NbtReader2 nbtReader, Registry<T> registry) {
        // x,y,z,t,p: Codec.INT, required
        // i: Registry.XXX.codec, required
        byte seenMask = 0b00_000;  // pt_xyz
        ...
        while (true) {
            ...
            if (nbtReader.matchesString(STRING_CHAR_SMALL_P)) {
                p = nbtReader.getIntOrThrow(tag);
                seenMask |= 0b10_000;
            } else if (nbtReader.matchesString(STRING_CHAR_SMALL_T)) {
                t = nbtReader.getIntOrThrow(tag);
                seenMask |= 0b01_000;
            } else if (nbtReader.matchesString(STRING_CHAR_SMALL_X)) {
                x = nbtReader.getIntOrThrow(tag);
                seenMask |= 0b00_100;
            } else if (nbtReader.matchesString(STRING_CHAR_SMALL_Y)) {
                y = nbtReader.getIntOrThrow(tag);
                seenMask |= 0b00_010;
            } else if (nbtReader.matchesString(STRING_CHAR_SMALL_Z)) {
                z = nbtReader.getIntOrThrow(tag);
                seenMask |= 0b00_001;
            } else if (nbtReader.matchesString(STRING_CHAR_SMALL_I)) {
                i = nbtReader.readRegistryOrThrow(registry);
            } else {
                nbtReader.skipCompoundEntry();
            }
        }
