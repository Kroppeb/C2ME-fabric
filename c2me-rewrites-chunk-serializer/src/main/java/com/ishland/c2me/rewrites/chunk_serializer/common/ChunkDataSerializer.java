...
public final class ChunkDataSerializer {
    ...
    private static final byte[] STRING_C2ME = getAsciiStringBytes("C2ME");
    private static final byte[] STRING_KROPPEB = getAsciiStringBytes("Kroppeb was here :); Version: 0.3.0");

    private static final byte[] STRING_C2ME_MARK_A = getAsciiStringBytes("C2ME::MarkA");
    private static final byte[] STRING_MARKER_FLUID_PROTO = getAsciiStringBytes("fluid:proto");
    private static final byte[] STRING_MARKER_FLUID_FULL = getAsciiStringBytes("fluid:full");
    private static final byte[] STRING_MARKER_FLUID_FALLBACK = getAsciiStringBytes("fluid:fallback");

    // STARLIGHT
    private static final byte[] STRING_BLOCKLIGHT_STATE_TAG = getAsciiStringBytes("starlight.blocklight_state");
    private static final byte[] STRING_SKYLIGHT_STATE_TAG = getAsciiStringBytes("starlight.skylight_state");
    private static final byte[] STRING_STARLIGHT_VERSION_TAG = getAsciiStringBytes("starlight.light_version");
...
//    }
    ...
    private static void writeStructures(
        ...
        for (var entry : starts.entrySet()) {
            writer.startCompound(getNameBytesFromRegistry(configuredStructureFeatureRegistry, entry.getKey()));
        ...
        for (var entry : references.entrySet()) {
            ...
            writer.putLongArray(getNameBytesFromRegistry(configuredStructureFeatureRegistry, entry.getKey()), entry.getValue());
        }
        writer.finishCompound();
